...
- JavaScript source snap
    ...
    const correctMarkdownLinks = (content) => {
    	...
    	const linkFloaterMatches = [[]](\))?)/gi);
    	const linkNotionMatches = [[]]
     ...
     * @param {string} link Markdown link of format [title](link/image.png)
...
Solved by the script above:
- Names all include UUIDs
- Summaries and other properties don't really work.
- Creation dates will all reflect export date
- Tags don't port natively.
